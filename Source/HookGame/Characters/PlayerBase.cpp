...
void APlayerBase::BeginPlay()
{
	...
	VaultInit();
	SkillTreeInit(PlayerController);
	PlayerHUDInit();
	LoadPlayerData();
	ApplyUpgrades();
}
void APlayerBase::Tick(float DeltaTime)
{
	...
	ToolTipTextUpdate(DeltaTime);
	CheckPointWidget(DeltaTime);
	LevelUpWidgetUpdate();
...
void APlayerBase::CameraRotationUpdate(const float DeltaTime)
{
	...
	auto RotateFunction = AssociatedTriggerBox->_RotateFunction;
	auto const RotateCamera = [&DeltaTime, &RotateFunction](const FRotator &Current, const FRotator &Target, const float Speed)
	{
		if (RotateFunction == RotateFunction::Constant)
			return FMath::RInterpConstantTo(Current, Target, DeltaTime, Speed);
		else
			return FMath::Lerp(Current, Target, Speed * DeltaTime);
	};
...
void APlayerBase::HookFire()
{
	...
	if (HookFireSound)
		UGameplayStatics::PlaySoundAtLocation(GetWorld(), HookFireSound, Hook->GetActorLocation());
...
void APlayerBase::SkillTreeInit(APlayerController* PlayerController)
{
	if (SkillTreeFCRI)
...
void APlayerBase::WidgetInit(_Inout_ APlayerController* PlayerController)
{
	...
	PlayerHUD = Cast<UMainHUD>(CreateWidget(PlayerController, MainHUDClass));
	...
	LogIfFailed(PlayerHUD);
	LogIfFailed(WidgetCursorNotLocked);
	LogIfFailed(WidgetCursorLocked);
	LogIfFailed(WidgetMenu);
	LogIfFailed(WidgetSword);
...
void APlayerBase::OnShiftPressed()
{
	bIsRunning = true;
}

void APlayerBase::OnShiftReleased()
{
	bIsRunning = false;
}
	...
	FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (HookHitSound->IsPlayable())
		UGameplayStatics::PlaySoundAtLocation(GetWorld(), HookHitSound, Hook->GetActorLocation());
...
void APlayerBase::OnUsePressed()
{
	...
	if (!bGodMode)
	{
		GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Green, TEXT("GodModOn"));
		bGodMode = true;
	}else
	{
		GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Green, TEXT("GodModOff"));
		bGodMode = false;
	}
}
...
void APlayerBase::CursorUpdate(const float DeltaTime)
{
	...
	if (bHit)
	{
		...
		bHit *= GetWorld()->LineTraceSingleByChannel(SceneHit, HookLocation, HookLocation + (HookToImpactDir * (HookMaxRange + 5000.f)), ECollisionChannel::ECC_Visibility, Params);
...
void APlayerBase::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction(TEXT("Shift"), IE_Pressed, this, &THIS::OnShiftPressed);
	PlayerInputComponent->BindAction(TEXT("Shift"), IE_Released, this, &THIS::OnShiftReleased);
...
void APlayerBase::MovementUpdate(const float DeltaTime)
{
	GetCharacterMovement()->MaxWalkSpeed = InitialPlayerSpeed;
	
	if (bIsRunning && CompareAttackState(AttackState::Default) && CompareHookState(HookCurrentState::GrappleDetached))
...
void APlayerBase::ApplyUpgrades()
{
	if (SkillTreeFCRI)
		SkillTreeFCRI->ApplyUpgrades();
}
...
void APlayerBase::CreateComponents()
{
	...
	LogIfFailed(CameraComponent);
	LogIfFailed(SpringArm);
	LogIfFailed(VaultCapsule);
	LogIfFailed(VaultBoxExtent);
	LogIfFailed(SkillTreeFCRI);
	LogIfFailed(ShadowProjectionCamera);
	LogIfFailed(DepthProjectionCamera);
	LogIfFailed(GrappleCable);
	LogIfFailed(ShadowProjectionCameraArm);	
}
